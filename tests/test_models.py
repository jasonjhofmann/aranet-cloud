...
def test_sensor_parses_full_sensor_list(sensors_payload):
    ...
    # find Primary Bedroom by name
    pb = next(s for s in sensors if s.name == "Primary Bedroom")
    assert pb.id == "4205836"
    assert pb.serial == "02D0C"
    assert pb.type == "S4V1"
    assert "358151002573" in pb.bases
    assert pb.active_metrics == ["1", "2", "3", "4", "61", "62"]
    assert pb.primary_base == "358151002573"
...
def test_sensor_multi_probe_parses(sensors_payload):
    """The S6V1 WET150 (Unhappy Lisbon Lemon Soil) has multiple probes."""
    sensors = [Sensor.from_dict(s) for s in sensors_payload["sensors"]]
...
def test_base_parses(bases_payload):
    ...
    assert len(bases) == 1
    b = bases[0]
    assert b.id == "358151002573"
    assert b.name == "Aranet-75ae36"
